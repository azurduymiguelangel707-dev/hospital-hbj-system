...
const DIAGNOSTICOS_COMUNES = [
  ...
  'J06 Infección Respiratoria Aguda',
  'K29 Gastritis',
  'N39 Infección de Tracto Urinario',
  'B02 Herpes Zoster',
  'M54 Gonalgia',
...
export function SNISPanel() {
  ...
  function exportarExcel() {
    const rows = [
      ...
      [`MÉDICO: ${medico}`, `N° MATRÍCULA: ${nMatricula}`],
      [],
      ['N° HC','N° ASEGURADO','APELLIDO PATERNO, MATERNO Y NOMBRES','TIPO PAC','EDAD M','EDAD F','PESO (Kg.)','TALLA (cm.)','CONS. N','CONS. R','DESNUT. MOD','DESNUT. GRAVE','SOBREPESO','NORMAL','AREAS DEL MES','CON MES','REPETIDAS','CONTROL PRENATAL','48H POST PARTO','DIABETES','HIPERTENSIÓN','ITS-VIH-SIDA','EXAM. MAMA','PAP','TUBERCULOSIS','REFERENCIA','CONTRARREF.','CIE 10','DIAGNÓSTICO'],
      ...filas.map((f,i) => [
    ...
    <div className="flex flex-col h-full overflow-hidden" ref={printRef}>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4 mb-3 flex-shrink-0">
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1">
            ...
            <p className="text-xs text-gray-500 mt-0.5">Hospital Municipal Modelo Boliviano Japonés</p>
          </div>
          {/* Acciones */}
        ...
        <div className="grid grid-cols-4 gap-3">
          ...
          <div>
            <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide block mb-1">Nombre del Médico</label>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide block mb-1">N° Matrícula</label>
      ...
      <div className="grid grid-cols-5 gap-2 mb-3 flex-shrink-0">
        {[
          ...
          { label: 'Hipertensión', val: totalHTA, color: '#ef4444', bg: '#fef2f2' },
        ].map((k,i) => (
          ...
          <table className="text-xs border-collapse" style={{ minWidth: '1800px' }}>
            <thead className="sticky top-0 z-10">
              <tr className="bg-blue-800 text-white">
                ...
                <th className="border border-blue-700 px-1 py-1.5 text-center" colSpan={6}>Detección de Enfermedades</th>
                ...
                <th className="border border-blue-700 px-1 py-1.5 text-center w-48" rowSpan={2}>Diagnóstico</th>
              ...
              <tr className="bg-blue-700 text-white">
                ...
                <th className="border border-blue-600 px-1 py-1 text-center w-12">Áreas del Mes</th>
                ...
                <tr key={f.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-blue-50/30'}>
                  ...
                  <td className="border border-gray-200 px-0.5 py-0.5">
                    <input value={f.diagnostico} onChange={e => actualizarFila(f.id,'diagnostico',e.target.value)}
                      list={`diag-${f.id}`}
                      className="w-full px-1 py-0.5 text-xs focus:outline-none focus:bg-blue-50 rounded" placeholder="Diagnóstico" />
