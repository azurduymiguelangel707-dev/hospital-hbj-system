...
function LineChartVitales({ datos, color, unidad, min, max }) {
  return (
...
function FichaPaciente({ paciente, onBack }: { paciente: any; onBack: () => void }) {
  ...
  useEffect(() => {
    const cargar = async () => {
      try {
        ...
        setHistorial(Array.isArray(hRes) ? hRes : (Array.isArray(hRes && hRes.data) ? hRes.data : []));
  ...
  const proximaCita = summary && summary.citas ? summary.citas.proxima : null;
  const ultimaVisita = summary && summary.citas ? summary.citas.ultimaVisita : null;
  const alertas = summary && summary.alertas ? summary.alertas : [];
  const METRICAS = [
  ...
  const metricaActual = METRICAS.find(function(m) { return m.key === metricaVital; }) || METRICAS[0];
  const datosGrafico = vitalesHistory.filter(function(v) { return v[metricaVital] !== null && v[metricaVital] !== undefined; }).map(function(v) { return { fecha: new Date(v.fecha).toLocaleDateString('es-ES', { day: '2-digit', month: 'short' }), valor: Number(v[metricaVital]) }; });
  const tabs = [
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          ...
          {alertas.length > 0 ? (
            <div className="space-y-1">
              {alertas.map(function(a, i) { return (
                  ...
                  {citasFull.map((c: any) => (
                    <div key={c.id} className="border border-gray-100 rounded-xl p-4 hover:border-gray-200 transition">
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <span className={'px-2.5 py-1 rounded-full text-xs font-semibold ' + (ESTADO_CONFIG[c.estado] ? ESTADO_CONFIG[c.estado].cls : 'bg-gray-100 text-gray-600')}>
                          {ESTADO_CONFIG[c.estado] ? ESTADO_CONFIG[c.estado].label : c.estado}
                        </span>
                      </div>
                      <div className="grid grid-cols-3 gap-2 text-xs text-gray-500 mt-2 pt-2 border-t border-gray-50">
                        <span><span className="text-gray-400">Doctor:</span> {c.doctor ? c.doctor.nombre : 'Sin asignar'}</span>
              ...
              {tab === 'vitales' && (
                ...
                <div>
                  <div className="flex gap-2 mb-4 flex-wrap">
                    {METRICAS.map(function(m) { return (
                      <button key={m.key} onClick={function() { setMetricaVital(m.key); }}
                  ...
                  {datosGrafico.length === 0 ? <p className="text-center text-gray-400 py-6 text-sm">Sin datos para esta metrica</p> :
                    <LineChartVitales datos={datosGrafico} color={metricaActual.color} unidad={metricaActual.unidad} min={metricaActual.min} max={metricaActual.max} />}
                  <div className="mt-4 space-y-2">
                    <p className="text-xs text-gray-400 uppercase tracking-wide mb-2">Ultimos registros</p>
                    {vitalesHistory.slice(0, 5).map(function(v, i) { return (
