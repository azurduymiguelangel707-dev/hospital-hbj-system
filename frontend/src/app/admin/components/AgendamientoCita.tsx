      ...
      <div className="bg-green-50 border border-green-200 rounded-xl p-6">
        ...
        <div className="bg-white border border-green-200 rounded-xl p-5 mb-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: 'NÂ° Ficha', value: `${saved.numeroFicha} / ${saved.totalFichasTurno}`, highlight: true },
              { label: 'NÂ° Historial', value: patient.numeroHistorial, highlight: true },
          ...
          <div className={`rounded-lg px-4 py-3 border text-sm ${slots.fichasDisponibles > 0 ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
            {loadingSlots ? (
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-semibold text-green-700">Ficha NÂ° {slots.proximaFicha} / {slots.totalFichas}</p>
              ...
              <p className="text-red-700 font-medium">Turno completo â€” sin fichas disponibles</p>
            )}
          </div>
        )}
            ...
            <select value={form.doctorId} onChange={e => setForm(f => ({ ...f, doctorId: e.target.value }))}
              ...
              {doctors.map(d => (
                <option key={d.id} value={d.id}>
                  Dr. {d.user?.first_name ?? ''} {d.user?.last_name ?? ''} â€” {d.specialty}
                </option>
              ))}
            </select>
          )}
