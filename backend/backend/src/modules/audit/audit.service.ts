...
export class AuditService {
  ...
  async logEvent(params: {
    ...
    const { hash, nonce } = this.mineBlock(newBlock, previousHash);
  ...
  private mineBlock(block: Partial<AuditLog>, previousHash: string, difficulty: number = 2): { hash: string; nonce: number } {
    let nonce = 0;
    let hash = '';
